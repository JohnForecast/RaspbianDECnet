...
static int __dn_connect(struct sock *sk, struct sockaddr_dn *addr, int addrlen, long *timeo, int flags)
{
	...
	dstaddr = le16_to_cpu(dn_saddr2dn(addr));
	 ...
	 */
	if (dstaddr == 0)
		dstaddr = le16_to_cpu(decnet_address);
	else if ((dstaddr & 0xFC00) == 0)
		dstaddr |= (le16_to_cpu(decnet_address) & 0xFC00);
	else if ((dstaddr & 0x3FF) == 0)
		goto out;

	memcpy(&scp->peer, addr, sizeof(struct sockaddr_dn));

	*(__le16 *)&scp->peer.sdn_nodeaddr = cpu_to_le16(dstaddr);
...
"NET4: DECnet for Linux: V.2.6.02a (C) 1995-2003 Linux DECnet Project Team\n";
